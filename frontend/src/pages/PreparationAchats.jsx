...
export function PreparationImportation({ canEdit, canDelete = false, showMsg, onOpenReception }) {
  ...
  const transfer = async (id) => { try { const result = await api.request(`/warehouse/preparations-importation/${id}/basculer-reception`, { method: 'POST' }); await api.request('/data/doc/is_import_reception_draft', { method: 'PUT', body: JSON.stringify(result.receptionDraft) }); showMsg('Brouillon transféré : pointez maintenant les pièces à la réception.', 'success'); onOpenReception?.(); } catch (error) { showMsg(error.message, 'error'); } };
